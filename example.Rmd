...
`knitr`. A selection of tutorials and useful examples for `knitr` can be found on 
[`knitr`'s homepage](http://yihui.name/knitr/demo/showcase/). However, when trying
to use this to generate publication quality reports the limitations of the
Markdown syntax quickly start to become apparent. The focus on simplicity and the 
fact that it was originally designed for authoring web content means that much
of the requirements for scientific writing are not easily met by standard Markdown.

`Pandoc` is a very useful tool that helps to alleviate this problem. It comes with its 
own Markdown dialect that includes many extensions that fill some of the gaps in the
